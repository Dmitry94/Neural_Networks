...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        'Demo applications for demonstration SoftmaxClassifier work.')
...
def main():
    ...
    classifier = SoftmaxClassifier(args.lr, args.reg_lambda, args.bsize)
    ...
    #
    print('Testing implemented softmax classifier...')
    # Train
    start = time.clock()
    classifier.train(train_data, train_labels)
    end = time.clock()
    print('Softmax training time = {}'.format(end - start))
    # Predict
    start = time.clock()
    predictions = classifier.predict(test_batch['data'])
    end = time.clock()
    print('Softmax predicting time = {}'.format(end - start))
    accuracy = np.mean(predictions == test_batch['labels'])
    print('Softmax accuracy = {}'.format(accuracy))
